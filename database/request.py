...
def get_request(current_session, bot, chat_id, sub_tab, task_tab, mode_tab):
    # список доступных предметов
    bot.send_message(chat_id, 'Введите номер предмета из списка:\n'
                              '1 – Модели данных')  # переделать в дальнейшем
    new_subject_ans = int(input('Ведите номер предмета: '))  # тут надо отправлять пользователю

    # запросы из табличек тут
    bot.send_message(chat_id, 'Введите номер варианта из списка:\n'
                              '1 – Макет')  # переделать в дальнейшем
    task_name_ans = int(input('выберете номер варианта: '))  # тоже и с режимом и с названием задания
    bot.send_message(chat_id, 'Выберите режим, как пройдет помощь:\n'
                              '1 – лично')  # расширить
...
def search_student(current_session, bot, chat_id, sub_tab, task_tab, mode_tab, student_base, user_base):

    new_subject, task_name, task_mode = get_request(current_session, bot, chat_id, sub_tab, task_tab, mode_tab)
    result = []
...
def search_tutor(current_session, bot, chat_id, sub_tab, task_tab, mode_tab, tutor_base, user_base):
...
def get_my_requests(session, telega_id):  # 2
    requests = session.query(Tutor).filter(Tutor.user_id == in_db_id(session, telega_id)).all()
...
def get_my_open_deals(session, telega_id):  # 5,4
    ...
    for row in deals:
        if row.User.id == in_db_id(session, telega_id) and row.Deal.status == 'open':
            result.append(row)

    return result  # возвращает список из объектов класса если пустой, то сделок нет


def add_tutor(session, bot, telega_id):
    task_price = input('что вы хотите за выполнение?: ')   # проработать тут связь с пользователем
    user = get_user(session, telega_id)
    new_subject, task_name, task_mode = get_request(session, bot, telega_id, Subject, Task, Mode)
...
def add_student(session, bot, telega_id):
    user = get_user(session, telega_id)
    new_subject, task_name, task_mode = get_request(session, bot, Subject, Task, Mode)
...
def search(session, bot, chat_id, stud_or_tutor):
    if stud_or_tutor == 1:
        return search_student(session, bot, chat_id, Subject,  Task, Mode, Student, User)
    else:
        return search_tutor(session, bot, chat_id, Subject, Task, Mode, Tutor, User)
